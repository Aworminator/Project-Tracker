  ...
  async (req, res) => {
    try {
      ...
      const updateData = {
        ...
        deadline: end_date || null, // Map end_date to deadline
        updated_at: new Date().toISOString(),
      };
...
app.get("/api/dashboard/stats", requireAuth, async (req, res) => {
  try {
    ...
    // Also get all tasks to see what exists
    const { data: allTasks } = await supabase
      .from("tasks")
      .select("id, title, assigned_to, status");

    console.log("Stats Debug - User ID:", req.user.id);
    console.log("Stats Debug - User Tasks:", userTasks);
    console.log("Stats Debug - All Tasks:", allTasks);
    console.log("Stats Debug - Projects:", projects);
    ...
    res.json({
      ...
      tasks: {
        ...
        inProgress:
          userTasks?.filter((t) => t.status === "in-progress").length || 0,
            ...
            .insert([
              {
                email: email,
                password: hash,
                role: "manager", // Default role for new users - gives them project creation capabilities
